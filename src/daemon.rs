...
pub fn setup_daemon(args: Vec<String>) -> crate::Result<()> {
    ...
    let manager = <dyn ServiceManager>::native()
        .map_err(|e| color_eyre::eyre::eyre!("Failed to get native service manager: {}", e))?;

    // 1. Forcibly stop and uninstall existing service
    println!("Stopping existing service if running...");
    let _unused = manager.stop(ServiceStopCtx { 
        label: label.clone(),
    });

    println!("Uninstalling existing service...");
    let _unused = manager.uninstall(ServiceUninstallCtx {
        label: label.clone(),
    });
    ...
    let service_args: Vec<OsString> = args.into_iter()
        .skip(1)
        .filter(|a| a != "-d" && a != "--daemon")
        .map(OsString::from)
        .collect();

    println!("Installing service: {} {:?}", current_exe.display(), service_args);
