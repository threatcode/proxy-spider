...
fn test_parse_proxy_with_special_chars_in_password() {
    // Note: The regex has limitations on special characters in passwords
    // Testing with alphanumeric and basic special chars
    let text = "http://user:Pass123@192.168.1.1:8080";
    let captures: Vec<_> = PROXY_REGEX.captures_iter(text).collect();
    ...
    assert_eq!(capture.name("username").unwrap().as_str(), "user");
    assert_eq!(capture.name("password").unwrap().as_str(), "Pass123");
}
...
fn test_parse_proxy_from_mixed_content() {
    ...
    let captures: Vec<_> =
        PROXY_REGEX.captures_iter(text).filter_map(Result::ok).collect();
